...
def nearest_reference_diagnostics(
    ...
    *,
    ...
    generated_normalized = (
        generated[:, active] - reference_mean[active]
    ) / reference_std[active]
    reference_normalized = (
        reference[:, active] - reference_mean[active]
    ) / reference_std[active]
...
def _correlation(first: np.ndarray, second: np.ndarray) -> float:
    ...
    denominator = np.sqrt(
        float(np.sum(left_centered**2)) * float(np.sum(right_centered**2))
    )
    if denominator <= 0.0:
        return float("nan")
