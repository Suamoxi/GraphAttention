...
The source artifacts use one shared 2-D reference coordinate mesh for x-, y-, and z-normal slices. For the geometry-aware M9 model, those coordinates are embedded back into the original global Cartesian frame using the authoritative slice metadata:

$$
\begin{aligned}
x\text{-normal: } & r=(x_{\mathrm{slice}},y,z),\\
y\text{-normal: } & r=(x,y_{\mathrm{slice}},z),\\
z\text{-normal: } & r=(x,y,z_{\mathrm{slice}}).
\end{aligned}
$$

This keeps the M9 displacement components in the same global frame as `rhou`, `rhov`, and `rhow`.
