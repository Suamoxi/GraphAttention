...
| Authoritative declared case reference documents | Project scientific/provenance convention | `docs/M3_3_NONDIMENSIONALIZATION.md` §10-11 | `CaseDefinition`, `load_case_definition`, `AVBPHDF5Dataset.case_definitions`, `Sample.case_id` | `tests/unit/test_case_definition.py`, `tests/unit/test_avbp_hdf5.py` | 55-test integration passed on Calypso; final combined M3.3 validation pending |
| Explicit reference values with non-evaluated derivation metadata | Project reproducibility convention | `docs/M3_3_NONDIMENSIONALIZATION.md` §10 | `load_case_definition` | literal-value and schema failure tests | Implemented; final combined M3.3 validation pending |
| Baseline coordinate nondimensionalization `x/L_ref` | Project scientific/numerical convention | `docs/SCIENTIFIC_SPEC.md` §8.1, `docs/NUMERICAL_CONVENTIONS.md` §13, `docs/M3_3_NONDIMENSIONALIZATION.md` §4, §13 | `ConvectiveNondimensionalizer.nondimensionalize_coordinates`, `dimensionalize_coordinates` | coordinate scale, failure, and round-trip tests in `tests/unit/test_nondimensionalization.py` | Implemented M3.3; target validation pending |
| `Re`, `Ma`, and related quantities as regime descriptors, not primary normalization scales | Project scientific convention | `docs/M3_3_NONDIMENSIONALIZATION.md` §5, §10, `docs/SCIENTIFIC_SPEC.md` §10 | `RegimeParameter`, `RegimeParameters`, `CaseDefinition.regime_parameters`, `Sample.regime_parameters` | contract, case-file, and AVBP attachment tests | Persistence implemented M3.3; model conditioning deferred; target validation pending |
