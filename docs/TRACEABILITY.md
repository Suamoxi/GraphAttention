...
| Generic case-level reference-state directive | Project scientific convention using coherent convective scaling | `docs/M3_3_NONDIMENSIONALIZATION.md`, `docs/SCIENTIFIC_SPEC.md` §8-11 | `ReferenceScale`, `ReferenceScope`, `ReferenceScales`, `ConvectiveNondimensionalizer` | `tests/unit/test_data_contracts.py`, `tests/unit/test_nondimensionalization.py` | Field-transform runtime validated on Calypso 2026-09-02 |
| Authoritative declared case reference documents | Project scientific/provenance convention | `docs/M3_3_NONDIMENSIONALIZATION.md` §10-11 | `CaseDefinition`, `load_case_definition`, `AVBPHDF5Dataset.case_definitions`, `Sample.case_id` | `tests/unit/test_case_definition.py`, `tests/unit/test_avbp_hdf5.py` | Implemented M3.3; target validation pending |
| Explicit reference values with non-evaluated derivation metadata | Project reproducibility convention | `docs/M3_3_NONDIMENSIONALIZATION.md` §10 | `load_case_definition` | literal-value and schema failure tests | Implemented M3.3; target validation pending |
| Baseline coordinate nondimensionalization `x/L_ref` | Project scientific/numerical convention | `docs/SCIENTIFIC_SPEC.md` §8.1, `docs/NUMERICAL_CONVENTIONS.md` §13, `docs/M3_3_NONDIMENSIONALIZATION.md` §4 | runtime transform not implemented | scientific definition review | Definition frozen; runtime deferred |
| `Re`, `Ma`, and related quantities as regime descriptors, not primary normalization scales | Project scientific convention | `docs/M3_3_NONDIMENSIONALIZATION.md`, `docs/SCIENTIFIC_SPEC.md` §10 | model conditioning not implemented | scientific definition review | Scientific definition frozen M3.3; persisted conditioning contract deferred |
...
| Explicit case-level reference semantics | Project scientific contract | `docs/SCIENTIFIC_SPEC.md` §8-11, `docs/M2_DATA_CONTRACTS.md`, `docs/M3_3_NONDIMENSIONALIZATION.md` | `ReferenceScale`, `ReferenceScope`, `ReferenceScales.scheme`, `CaseDefinition` | reference-contract, case-definition, and nondimensionalization tests | M3.3 implemented; new case-file path target validation pending |
| Physical nondimensionalization before statistical scaling | Project scientific/numerical convention | `docs/SCIENTIFIC_SPEC.md` §8-12, `docs/NUMERICAL_CONVENTIONS.md` §8-10, `docs/M3_3_NONDIMENSIONALIZATION.md` | `ConvectiveNondimensionalizer` | explicit-scale and round-trip tests | Field transform validated on Calypso; statistical scaling deferred until task/split contracts |
| Inference-available reference quantities only | Project anti-leakage rule | `docs/SCIENTIFIC_SPEC.md` §9, `docs/M3_3_NONDIMENSIONALIZATION.md` | case-file schema + reference validation in `ConvectiveNondimensionalizer` | unavailable/snapshot/schema failure tests | M3.3 baseline enforced; task-specific snapshot exception deferred |
...
| Data/Geometry/Task/Model/Trainer ownership | Project software-science design | `docs/ARCHITECTURE.md` §3 | package architecture; native data, declared references, and geometry transforms remain separated | review/tests | Frozen M0 / instantiated M1-M3.3 |
