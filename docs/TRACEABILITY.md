...
| Explicit case-level reference semantics | Project scientific contract | `docs/SCIENTIFIC_SPEC.md` §8-11, `docs/M2_DATA_CONTRACTS.md`, `docs/M3_3_NONDIMENSIONALIZATION.md` | `ReferenceScale`, `ReferenceScope`, `ReferenceScales.scheme` | reference-contract and nondimensionalization tests | M3.3 runtime semantics implemented; target validation pending |
| Physical nondimensionalization before statistical scaling | Project scientific/numerical convention | `docs/SCIENTIFIC_SPEC.md` §8-12, `docs/NUMERICAL_CONVENTIONS.md` §8-10, `docs/M3_3_NONDIMENSIONALIZATION.md` | `ConvectiveNondimensionalizer` | explicit-scale and round-trip tests | M3.3 runtime implementation; target validation pending |
| Inference-available reference quantities only | Project anti-leakage rule | `docs/SCIENTIFIC_SPEC.md` §9, `docs/M3_3_NONDIMENSIONALIZATION.md` | reference validation in `ConvectiveNondimensionalizer` | unavailable/snapshot reference failure tests | M3.3 baseline enforced; task-specific snapshot exception deferred |
...
| Data/Geometry/Task/Model/Trainer ownership | Project software-science design | `docs/ARCHITECTURE.md` §3 | package architecture; M3.2 separates native connectivity reading from graph-edge construction | review/tests | Frozen M0 / instantiated M1-M3.3 |
