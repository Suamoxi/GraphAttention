"""Generate HIT 2-D slices from an Improved-DDPM run."""
...
def main(cfg: DictConfig) -> None:
    run_improved_diffusion_generation(cfg)


def run_improved_diffusion_generation(cfg: DictConfig) -> dict[str, Any]:
    """Generate one unpaired test-sized population from an Improved-DDPM run."""
    ...
    for path, label in (
        ...
        if not path.is_file():
            raise FileNotFoundError(f"source Improved-DDPM {label} does not exist: {path}")
    ...
    if device.type == "cuda" and not torch.cuda.is_available():
        raise RuntimeError("CUDA Improved-DDPM generation requested but CUDA is unavailable")
    if torch.distributed.is_available() and torch.distributed.is_initialized():
        raise RuntimeError("Improved-DDPM generation is intentionally single-process")
    ...
    if not isinstance(dataset, PrecomputedSlicePTDataset):
        raise TypeError("source Improved-DDPM run must use data=hit_slice_pt")
    ...
    if output_dir.exists() and not bool(cfg.overwrite):
        raise FileExistsError(
            f"Improved-DDPM generation output already exists: {output_dir}; "
    ...
    summary = {
        ...
        "reverse_variance_parameterization": "learned_range_log_interpolation",
