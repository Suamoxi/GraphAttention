"""Benchmark M9 relative-displacement geometric sparse attention."""
...
from benchmark_m8 import _measurement_payload
from graph_attention.data import SplitManifest
from graph_attention.models import GeometricSparseGraphTransformer
...
def main() -> None:
    ...
    model = GeometricSparseGraphTransformer(
        ...
        spatial_dim=device_batch.coords.shape[1],
    ...
    result = {
        "benchmark": "M9_geometric_sparse_graph_transformer",
        ...
        "execution": {
            ...
            "geometry_feature": "relative_displacement_source_minus_target_only",
            "geometry_bias": "per_layer_mlp_to_per_head_additive_attention_bias",
            "geometry_mlp_hidden_dim": args.num_heads,
        ...
        "notes": [
            "M9 uses the same supplied sparse topology and value aggregation as M8.",
            "The only new attention signal is source-minus-target relative displacement.",
            "No explicit distance, direction normalization, or absolute coordinate is added.",
            "For physically nondimensionalized tasks, coordinates are already scaled by L_ref.",
