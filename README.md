...
- [`docs/M8_SPARSE_TRANSFORMER.md`](docs/M8_SPARSE_TRANSFORMER.md): first true sparse one-hop transformer, scientific equations, validation, and benchmark evidence.
- [`docs/M9_GEOMETRIC_ATTENTION.md`](docs/M9_GEOMETRIC_ATTENTION.md): relative-displacement geometric attention bias and its validation/benchmark gate.
- [`docs/M10_HIT_SLICE_ABLATION.md`](docs/M10_HIT_SLICE_ABLATION.md): first controlled M8-vs-M9 learning ablation on precomputed HIT 2-D slices.
- [`docs/M11_FLOW_MATCHING.md`](docs/M11_FLOW_MATCHING.md): unconditional five-channel HIT-slice flow matching with sparse graph Transformers.
...
M0-M2 are complete. M3.1 provides deterministic synthetic variable-mesh samples. M3.2 is complete and has been validated against a real AVBP HIT snapshot/mesh pair on Calypso. M3.3 is complete for its frozen baseline scope and target-validated on the real `HIT_LES_FORCED` case. M4 is complete and target-validated on Calypso. M5 is complete and target-validated on Calypso. M6 is complete and target-validated on Calypso with 122 passing tests plus the two-rank CPU/Gloo global-objective validation. M7 is complete and target-validated on a single NVIDIA GH200 on Calypso. M8 is complete and target-validated on the GH200 for the synthetic S3 and real `HIT_LES_FORCED` workloads; the additional degree diagnostic showed that extreme hub degree can dominate the native sparse backward runtime even at nearly fixed node/edge counts. M9 is software-validated and target-performance-validated on one GH200 in job `403939`; the relative-displacement geometry adds modest FP32 latency/memory overhead versus M8. M10 is software-validated with 182 passing tests, real-artifact validated on 1,488 fixed 2-D HIT slices, and one-epoch M8/M9 GH200 training-smoke validated in job `404059`; full learning-quality evidence remains pending. M11 now implements the first unconditional flow-matching pipeline for the full five-channel HIT slice state; software and target-training validation are pending.
